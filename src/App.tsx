...
function AuthGate() {
  ...
  useEffect(() => { if (session) { enableUserSync(session.user.id); void syncUserData(session) } }, [session])
  ...
  async function submit(event: FormEvent) {
    ...
    if (mode === 'sign-up') {
      if (!otpSent) {
        const { error: otpError } = await client.auth.signUp({ email, password })
        if (otpError) return setError(otpError.message)
        setOtpSent(true); setMessage('A 6-digit verification code was sent to your email. Verify it to finish creating your account.')
        return
      }
      const { error: verifyError } = await client.auth.verifyOtp({ email, token: otp, type: 'signup' })
