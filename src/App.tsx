...
import { createFlashcardSession, flashcardIntervals, formatReviewInterval, isAcceptedAnswer, isFlashcardRetryAt, isQuestionDue, LEVEL_NAMES, moveQuestion, prioritizeDueFlashcardRetry, randomSelection, recordAnswer, scheduleFlashcard } from './mastery'
...
function ReviewPage() {
  ...
  useEffect(() => {
    ...
    setFlashcardSession((session) => session ? prioritizeDueFlashcardRetry(session, index, new Date(reviewNow)) : session)
  }, [hasActiveFlashcardSession, index, mode, reviewNow])
  const queuedCurrent = pool[index]
  const waitingForAgain = mode === 'flashcards' && Boolean(queuedCurrent) && isFlashcardRetryAt(pool, index) && !isQuestionDue(queuedCurrent, new Date(reviewNow))
  const current = waitingForAgain ? undefined : queuedCurrent
  ...
  async function rateFlashcard(rating: ReviewRating) {
    ...
    try {
      ...
      setFlashcardSession((session) => {
        ...
        if (rating !== 'again') return updatedSession
  ...
  if (waitingForAgain && queuedCurrent) {
    const waitSeconds = Math.max(1, Math.ceil((new Date(queuedCurrent.reviewDueAt ?? reviewNow).getTime() - reviewNow) / 1_000))
    return <div className="page narrow"><Link className="back-link" to={`/subjects/${subjectId}?tab=review`}><ArrowLeft /> {subject.name} study modes</Link><section className="result-card"><span className="result-icon"><Clock3 /></span><p className="eyebrow">Again card queued</p><h1>{waitSeconds}s</h1><p>This card will appear automatically when its one-minute timer finishes. No refresh needed.</p></section></div>
  }
