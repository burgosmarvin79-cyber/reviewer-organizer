...
export async function syncUserData(userId: string) {
  ...
  try {
    ...
    const questions = remoteQuestions.data.map((item) => {
      ...
      return { id: item.id, subjectId: item.subject_id, prompt: item.prompt, acceptedAnswers: item.accepted_answers, explanation: item.explanation, level: item.level, totalAttempts: item.total_attempts, totalCorrect: item.total_correct, lastAnsweredAt: item.last_answered_at ?? undefined, reviewState: item.review_state ?? 'new', reviewIntervalMinutes: item.review_interval_minutes ?? 0, reviewEase: Number(item.review_ease ?? 2.3), reviewDueAt: item.review_due_at ?? undefined, reviewRepetitions: item.review_repetitions ?? 0, reviewLapses: item.review_lapses ?? 0, createdAt: item.created_at, updatedAt: item.updated_at }
    })
