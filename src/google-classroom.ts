...
interface ItemListResponse { courseWork?: ClassroomItem[]; courseWorkMaterial?: ClassroomItem[]; nextPageToken?: string }
...
async function listCourseItems(fetchApi: ClassroomFetcher, courseId: string, resource: 'courseWork' | 'courseWorkMaterials') {
  ...
  do {
    ...
    items.push(...(resource === 'courseWork' ? data.courseWork ?? [] : data.courseWorkMaterial ?? []))
    pageToken = data.nextPageToken ?? ''
  } while (pageToken)
  return items
}
...
export async function listCourseAttachments(fetchApi: ClassroomFetcher, courseId: string) {
  ...
  return extractDriveAttachments([...coursework, ...materials])
}
